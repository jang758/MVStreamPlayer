...
> **최종 업데이트:** 2026-02-28  
> **상태:** **구간 다운로드(Bandicut-style) + 마우스 휠 볼륨 + 병렬 세그먼트 다운로드 추가 완료**  
> **사용자 환경:** Windows, Whale + Edge 브라우저 (Chrome 미설치), Python 가상환경(venv)  
> **코드 규모:** 약 8,400줄 (server.py ~3,370 / app.js ~2,540 / style.css ~2,120 / index.html ~326)
...
- **구간 다운로드**: Bandicut-style UI로 특정 구간만 다운로드 (HLS 세그먼트 직접 다운로드 + ffmpeg 리먹스)
...
| POST   | `/api/clip-download`         | 구간 다운로드 요청 (start/end/url/title) |
| GET    | `/api/clip-status/<uid>`     | 구간 다운로드 상태 (progress/detail/status) |
...
| 재생 속도        | 드롭다운 0.25x ~ 2x                                                                                         |
| 마우스 휠 볼륨   | 영상 위에서 휠 위=볼륨↑, 휠 아래=볼륨↓ (5% 단위), 슬라이더/음소거 자동 동기화                                |
| 볼륨 바 확장     | 140px 폭 (기존 70px의 2배)으로 미세 조정 가능                                                                |
...
### Phase 11: 구간 다운로드 + 볼륨 개선 (완료)

- **구간 다운로드 (Bandicut-style UI)**:
  - `✂️` 버튼으로 구간 다운로드 패널 토글. 시작/종료 시간 `HH:MM:SS` 입력 + 현재 위치 설정 버튼
  - 시각적 Range Bar에 드래그 가능한 마커(시작=초록 ▼, 끝=빨강 ▼) + 채움 바
  - Duration 실시간 표시, 마커↔입력필드 양방향 동기화
  - `/api/clip-download` (POST) + `/api/clip-status/<uid>` (GET) 엔드포인트
- **HLS 세그먼트 직접 다운로드 방식** (ffmpeg HLS 파서 완전 우회):
  - 문제: CDN이 영상 세그먼트를 `.jpeg` 파일로 위장 → ffmpeg가 `image2` 형식으로 거부
  - 해결: Python `requests`로 M3U8 파싱 → 필요 세그먼트만 프록시 통해 직접 다운로드 → `.ts` 파일로 합침 → ffmpeg는 로컬 파일에서 트리밍+MP4 리먹스만 수행
  - ffmpeg가 HLS 스트림을 직접 접근하지 않으므로 CDN 인증/형식 문제 완전 회피
- **4-스레드 병렬 세그먼트 다운로드** (`ThreadPoolExecutor(max_workers=4)`):
  - 기존 순차 다운로드 대비 ~4배 속도 향상
  - 완료 순서 무관하게 원본 순서 보장 (`seg_data[idx]` 배열)
- **구간 다운로드 실시간 진행률**:
  - 서버: `_clip_status[uid]["detail"]`에 `"42/215 세그먼트 (80.5MB)"` 형식 저장
  - 프론트: 1초 폴링으로 `⬇️ 42/215 세그먼트 (80.5MB) (33%)` 실시간 표시
- **마우스 휠 볼륨 조절**: 영상 위에서 휠 위=볼륨↑, 아래=볼륨↓ (5% 단위). 슬라이더/음소거 자동 동기화, 설정 자동 저장
- **볼륨 바 2배 확장**: CSS `width: 70px` → `140px`로 미세 조정 용이
- **M3U8 세그먼트 프록시 URL 변경**: `/api/ts-proxy?url=...` → `/api/ts-proxy/s.ts?url=...` (ffmpeg 호환성)
